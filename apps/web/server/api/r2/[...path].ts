...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    // Always set strong cache headers for static responses
    const headers = buildStaticCacheHeaders(3600);
    for (const [key, value] of Object.entries(headers)) {
      setHeader(event, key, value);
    }
    // Preserve upstream cache-control if provided (still overridden by CDN headers)
    if (cacheControl) {
