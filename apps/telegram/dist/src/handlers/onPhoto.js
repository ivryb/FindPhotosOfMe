...
export const createOnPhotoHandler = (convexUrl, collectionId, botToken) => {
    ...
    return async (ctx) => {
        ...
        const requestId = await httpClient.mutation("searchRequests:create", {
            collectionId,
        ...
        if (!r2ProxyBase) {
            await ctx.reply("Found results, but image proxy is not configured (R2_PROXY_BASE_URL).");
            return;
        }
        log("Subscribing for search result", { requestId: String(requestId) });
        const latest = await waitForSearch(convexUrl, requestId, timeoutMs);
        if (!latest) {
        ...
        const images = Array.isArray(latest.imagesFound)
            ? latest.imagesFound
            : [];
        await sendPhotoResults(ctx, images, r2ProxyBase);
    };
};
//# sourceMappingURL=onPhoto.js.map